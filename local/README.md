...
| Postgres | localhost:5432 | `atlas`/`atlas`/`atlas` (superuser — migrations + seeding). The gateway *runtime* connects as the non-superuser `atlas_app` so the BRA-887 RLS policies on `call_records`/`budgets` apply; `atlas_app` is created on fresh DB init (`local/initdb/`) — recreate it with `down -v` on an existing volume. |
